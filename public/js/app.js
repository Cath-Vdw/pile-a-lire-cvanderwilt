...
function updateLengthFilter() {
  ...
  if (format === "audio") {
    elements.lengthLabel.textContent = "Durée:";
    ...
    // Cache le label comic pour l'audio
    if (elements.lengthComicLabel) {
      elements.lengthComicLabel.style.display = "none";
    }
  } else {
    elements.lengthLabel.textContent = "Pages:";
    // Affiche le label comic seulement pour papier/ebook
    if (elements.lengthComicLabel) {
      elements.lengthComicLabel.style.display = "";
...
function getFilteredBooks() {
  return BOOKS_DATABASE.filter((book) => {
    ...
    if (currentFilters.length !== "all") {
      if (currentFilters.format === "audio") {
        ...
        if (
          ...
          return false;
        if (currentFilters.length === "long" && book.pages <= 500) return false;
      }
    }
    ...
    if (currentKeyword) {
      ...
      const matchGenre = book.genres.some((g) => g.toLowerCase().includes(kw));
...
function createBookCard(book) {
  ...
  likeBtn.addEventListener("click", (e) => {
    ...
    likeBtn.title = isNowLiked ? "Retirer des coups de cœur" : "Coup de cœur";
...
function openBookModal(book) {
  ...
  // Adapte le label "Pages" / "Durée" selon le format
  if (book.format === "audio") {
    elements.lengthLabel.textContent = "Durée:";
    elements.detailLength.textContent = book.duration;
  } else {
    elements.lengthLabel.textContent = "Pages:";
    elements.detailLength.textContent = book.pages;
  }
  ...
  const languageText = book.language === "en" ? "Anglais" : "Français";
  languageDisplay.textContent = `Langue: ${languageText}`;
  ...
  // Classe CSS et texte du bouton favori
  ...
  detailLikeBtn.onclick = () => {
    ...
    // Recrée l'icône Lucide et corrige le fill inline selon l'état
  ...
  // Régénère les icônes Lucide pour le nouveau bouton de cœur
  lucide.createIcons();
  ...
  if (archivedData && archivedData.rating) {
    ...
    elements.detailFavoriteBtn.closest(".detail-actions").before(reviewBlock);
  }
...
function updatePileDisplay() {
  ...
  const likedArchived = archived
    ...
    .sort((a, b) => {
      ...
      return (bookA?.title || "").localeCompare(bookB?.title || "", "fr", {
        sensitivity: "base",
      });
    });
  renderPileBooks(
...
function confirmRating() {
  const rating = document.querySelectorAll(".stars-rating .star.active").length;
