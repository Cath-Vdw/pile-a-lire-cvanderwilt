...
// BookRepository : accès à la base de données pour les livres (requêtes MySQL via Doctrine)
...
class BookController extends AbstractController
{
  ...
  // Chaque entrée a un 'id' (utilisé en interne / dans l'URL) et un 'name' (affiché à l'écran).
  ...
  public function index(BookRepository $bookRepository): Response
  {
    ...
    // Les clés ici doivent correspondre à celles attendues par app.js côté JavaScript.
    $booksData = array_map(fn($book) => [
      ...
      'genres'      => $book->getGenres(),       // tableau de genres (ex: ["fantasy", "adventure"])
      'summary'     => $book->getSummary(),
      'cover_color' => $book->getCoverColor(),   // couleur de fond utilisée si pas de couverture image
    ...
    // Récupère les statistiques globales (ex: nombre de livres lus, pages totales...).
    $stats = $bookRepository->getStatistics();

    // Envoie les données au template Twig books/index.html.twig.
    // - books_json  : chaîne JSON des livres, injectée dans le HTML pour être lue par app.js
    // - genres_list : liste des genres pour générer les filtres dans le template
